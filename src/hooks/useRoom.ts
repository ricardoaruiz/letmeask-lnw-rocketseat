...
type FirebaseQuestions = Record<string, Omit<Question, 'id' | 'likeCount' | 'likeId' > & {
...
export const useRoom = (roomId?: string): UseRommType => {
     ...
     */
    const refreshQuestions = React.useCallback((): firebase.database.Reference | undefined => {
        ...
        roomRef.on('value', room => {            
            ...
            const parseQuestions = Object.entries(questions).map(([key, value]) => {
                return {
                    ...
                    likeId: value.likes ? Object.entries(value.likes).find(([key, like]) => like.authorId === user?.id)?.[0] : undefined
                } as Question
            })
     ...
     */
    const createNewQuestion = React.useCallback((question: Omit<Question, 'id'> ): Promise<void> => {
    ...
    const removeQuestion = React.useCallback((questionId: string): Promise<void> => {
        ...
        return new Promise((resolve, reject) => {
            const questionsRef = database.ref(`rooms/${roomId}/questions/${questionId}`)
